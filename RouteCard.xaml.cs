...
namespace weldone
{
    ...
    {
        string[] sID, Autoitems, productList1, prodName, empList, machineList, toolList, process;
        ...
        private async void submit(object sender, RoutedEventArgs e)
        {
            if (fOrderNo.Text.Length > 0 && fSalesNo.Text.Length > 0 && fQuantityStrip.Text.Length > 0 && fStripSize.Text.Length > 0 && fCoilNo.Text.Length > 0 && fBatchNo.Text.Length > 0 && fDate.Text.Length > 0 && fTime.Text.Length > 0)
            {
                try
                {
                    ...
                    FirebaseResponse response1 = await client.GetAsync("Manufacturing/RouteCard/count/");
                    int rCount;
                    try
                    {
                        rCount = response1.ResultAs<int>();
                    }
                    catch
                    {
                        rCount = 0;
                    ...
                    FirebaseResponse response2 = await client.SetAsync("Manufacturing/RouteCard/count/", rCount);
                    FirebaseResponse response3 = await client.GetAsync("Manufacturing/Spinner/pending/count/");
                    int pCount = response3.ResultAs<int>();
                    pCount++;
                    FirebaseResponse firebaseResponse = await client.SetAsync("Manufacturing/Spinner/pending/count/", pCount);
                    FirebaseResponse response4 = await client.SetAsync("Manufacturing/Spinner/pending/" + pCount + "/", fOrderNo.Text);
                }
                catch (Exception)
                {
                    MessageDialog error = new MessageDialog("Failed to connect to database!");
    ...
    internal class employeeList1
    {
        public int SNo { get; set; }
        public string Name { get; set; }
    }
    internal class routeCard
    {
