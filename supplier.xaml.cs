    ...
    public sealed partial class supplier : Page
    {
        ...
        private async void Page_Loader(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                FirebaseResponse response = await client2.GetAsync("Master/Spinner/Supplier/count/");
                int c;
                try
                {
                ...
                for (int i = 1; i <= c; i++)
                {
                    FirebaseResponse response1 = await client2.GetAsync("Master/Spinner/Supplier/" + i + "/");
                    String a = response1.ResultAs<String>();
                    FirebaseResponse response2 = await client2.GetAsync("Master/Supplier/" + a + "/");
